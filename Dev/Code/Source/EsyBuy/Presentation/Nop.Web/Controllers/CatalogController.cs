...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BaseNopController
    {
                                    ...
                                    if (!product.CustomerEntersPrice)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    priceModel.Discount  = (oldPrice - finalPrice) * 100 / oldPrice;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (finalPriceWithoutDiscountBase != oldPriceBase && oldPriceBase > decimal.Zero)
                            model.ProductPrice.Discount = Math.Round((oldPrice - finalPriceWithoutDiscount) * 100 / oldPrice);
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            model.CustomProperties.Add("noFormat", command.noFormatting);
        ...
        public ActionResult TopMenu()
        {
            ...
            //var listcat = new List<CategorySimpleModel>(cachedModel.AsEnumerable());
            //var serializer = new System.Runtime.Serialization.DataContractSerializer(typeof(List<CategorySimpleModel>));
            //var sb = new System.Text.StringBuilder();
            //using (var writer = System.Xml.XmlWriter.Create(sb))
            //{
            //    serializer.WriteObject(writer, listcat);
            //    writer.Flush();
            //    var xml = sb.ToString();
            //}
        ...
        public ActionResult QuickView(int productId, int updatecartitemid = 0)
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted)
                return InvokeHttp404();
        ...
        [ChildActionOnly]
        public ActionResult ProductManufacturers()
        {
            var customerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                 .Where(cr => cr.Active).Select(cr => cr.Id).ToList();
            string cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_MANUFACTURERS_MODEL_KEY, "ALL", _workContext.WorkingLanguage.Id, string.Join(",", customerRolesIds), _storeContext.CurrentStore.Id);
            var cacheModel = _cacheManager.Get(cacheKey, () =>
            {
                var model = _manufacturerService.GetAllManufacturers()
                    .Select(x =>
                    {
                        ...
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(x.PictureId),
                            Title = string.Format(_localizationService.GetResource("Media.Manufacturer.ImageLinkTitleFormat"), x.Name),
                            AlternateText = string.Format(_localizationService.GetResource("Media.Manufacturer.ImageAlternateTextFormat"), x.Name)
                        };
                        m.PictureModel = pictureModel;
                        return m;
                    })
                    .ToList();
                return model;
        ...
        public ActionResult HomepageMobiles(int? productThumbPictureSize)
        {
            var products = _productService.GetAllProductsDisplayedOnHomePage();
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
        ...
        public ActionResult HomepageLaptops(int? productThumbPictureSize)
        {
            var products = _productService.GetAllProductsDisplayedOnHomePage();
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
        ...
        public ActionResult HomepageManufacturers(int? productThumbPictureSize)
        {
            var products = _productService.GetAllProductsDisplayedOnHomePage();
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
